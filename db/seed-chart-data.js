...
async function seedChartData() {
    console.log('🌱 Seeding chart data for the last 7 days...');
    ...
    try {
        ...
        const testSevenDaysAgo = testCurrentTime - (7 * 24 * 60 * 60);
        
        const testQuery = `SELECT * FROM klines_1h WHERE bucket >= $1 AND bucket <= $2 AND currency_code = $3`;
        const testResult = await client.query(testQuery, [
            new Date(testSevenDaysAgo * 1000), 
            new Date(testCurrentTime * 1000), 
            'SOL_USDC'
        ]);
        
        console.log(`📈 API test query returned ${testResult.rows.length} records`);
        if (testResult.rows.length > 0) {
            console.log('✅ Chart data is ready!');
        } else {
            console.log('❌ No data found for the requested time range');
        }
        
        console.log('🎉 Chart data seeding completed!');
